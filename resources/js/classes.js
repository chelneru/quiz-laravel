let invite_method = 'manual';
let current_modal_quiz_id =null;
let current_modal_participant_id =null;
let class_id_to_delete = null;
if ($('.invite-participants-page').length > 0) {
    Dropzone.autoDiscover = false;
    let myDropzone = new Dropzone('#invite-participants-form', {
        // paramName: "files",
...
$(document).ready(function () {
    ...
    $('.create-class-page .create-class-btn').on('click', function () {
        ...
        let is_valid = ValidateCreateForm();
        if (is_valid) {
    ...
    $(".copy-class-page .class-select").on('change', function () {
        let quiz_class = $('.class-select').find('option:selected').text();
    ...
    $('.invite-participants-page .new-participant-button').on('click', function () {
        let $new_participant_row = $('.participant_row').first().clone();
    ...
    $('.invite-participants-page .invite-participants-btn').on('click', function () {
        if (invite_method == 'manual') {
            let is_valid = ValidateManualInviteForm();
            if (is_valid == true) {
                $('.participant_row').each(function () {
                    ...
                    if (!($(this).find('#participant_first_name').val() == '' && $(this).find('#participant_last_name').val() == '' && $(this).find('#participant_email').val() == '')) {
                        let first_name_input = $("<input>").attr("type", "hidden").attr("name", "participant_first_name[]").val($(this).find('#participant_first_name').val());
                        let last_name_input = $("<input>").attr("type", "hidden").attr("name", "participant_last_name[]").val($(this).find('#participant_last_name').val());
                        let email_input = $("<input>").attr("type", "hidden").attr("name", "participant_email[]").val($(this).find('#participant_email').val());
                        $('#invite-participants-form').append(first_name_input,last_name_input, email_input);
                    }
                });
                let method = $("<input>").attr("type", "hidden").attr("name", "invite_method").val(invite_method);
            ...
            let invi_method = $("<input>").attr("type", "hidden").attr("name", "invite_method").val(invite_method);
    ...
    $("#class-select").on('change', function () {
        let class_id = $(this).find('option:selected').attr('value');
        ...
        $.post({
            ...
            let tabsInstance = M.Tabs.getInstance($('.tabs'));
    ...
    $('#modal2 .dismiss-participant').on('click',function () {
        let class_id = $('.edit-class-page').attr('data-class-id');
    ...
    $('.delete-class-menu-btn').on('click',function () {
        ...
        $('#class-delete-confirm-modal .confirm-btn').click(function () {
            let class_id = class_id_to_delete;
            //delete quiz
            $.post({
                url: '/delete-class',
                headers:
...
function DismissParticipant(participant_id,class_id) {
    $.post({
        ...
        data: {user_id:participant_id, class_ids: [class_id],ajax_call:true}
    }).done(function (data) {
        console.log("Success: " + data);
        let result = JSON.parse(data);
        if(result.status == true) {
...
function ValidateCopyForm() {
    ...
    if (!$('input[name=copy_teachers]').prop('checked') &&
        !$('input[name=copy_participants]').prop('checked') &&
        !$('input[name=copy_quizzes]').prop('checked')
    ) {
        is_valid = false;
...
function UnlinkQuizFromClass(id) {
    $.post({
        ...
        data: {quiz_id: id, just_unlink: true}
    }).done(function (data) {
        console.log("Success: " + data);
        let result = JSON.parse(data);
        if(result.status == true) {
